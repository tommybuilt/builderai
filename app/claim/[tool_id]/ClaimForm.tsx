...
export function ClaimForm({ toolId }: Props) {
    ...
    return (
      <div className="text-center py-2">
        ...
        <p className="text-sm text-zinc-600 dark:text-zinc-400">
          If that email submitted this tool, a sign-in link has been sent. The link expires in 1 hour.
        </p>
      </div>
    )
  }
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      {TURNSTILE_SITE_KEY ? (
        ...
        <div className="p-3 bg-amber-100 dark:bg-amber-900/30 border border-amber-300 dark:border-amber-800 rounded-lg text-sm text-amber-800 dark:text-amber-300">
          Verification widget not configured. Claim is temporarily unavailable.
        </div>
      )}
      ...
      <button
        ...
        className="w-full px-6 py-3 bg-primary-600 hover:bg-primary-500 disabled:bg-zinc-400 dark:disabled:bg-zinc-700 disabled:cursor-not-allowed rounded-lg font-semibold text-white transition-colors"
      >
        {loading ? 'Sending claim link...' : 'Send claim link'}
      </button>
    </form>
  )
}
